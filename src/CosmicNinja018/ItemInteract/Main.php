...
namespace CosmicNinja018\ItemInteract;
...
class Main extends PluginBase implements Listener {
    ...
    public function onJoin(PlayerJoinEvent $event) {
        ...
        if(!isset($itemArray[0]) || !isset($itemArray[1]) || !isset($itemArray[2])) {
                    $this->getLogger()->error("[ItemInteract] Config Error Make Sure To Use ID:META:COUNT!");
        return;
                }
        $player = $event->getPlayer();
    ...
    public function onDrop(PlayerDropItemEvent $event) {
        $item = $event->getItem();
        if($item->getNamedTag()->getTag("ItemInteractPlugin") != null)
            $event->cancel();
    }
    ...
    public function onTransaction(InventoryTransactionEvent $event) {
        ...
        foreach ($transaction->getActions() as $action) {
           if($action->getSourceItem()->getNamedTag()->getTag("ItemInteractPlugin") != null)
              $event->cancel();
       }
   }  
